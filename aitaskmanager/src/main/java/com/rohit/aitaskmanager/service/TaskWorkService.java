...
public class TaskWorkService {
    ...
    public TaskWorkService(TaskRepository taskRepository, UserRepository userRepository){
    ...
    public Page<TaskResponseDTO> filterTasks(Long userId,
        ...
        if (status != null && priority != null && category != null) {
            tasks = taskRepository.findByCreatedByIdAndStatusAndPriorityAndCategoryIgnoreCaseAndTaskType(userId, status, priority, category, TaskType.WORK, pageable);
        } else if (status != null && priority != null) {
            tasks = taskRepository.findByCreatedByIdAndStatusAndPriorityAndTaskType(userId, status, priority, TaskType.WORK, pageable);
        } else if (status != null && category != null) {
            tasks = taskRepository.findByCreatedByIdAndStatusAndCategoryIgnoreCaseAndTaskType(userId, status, category, TaskType.WORK, pageable);
        } else if (priority != null && category != null) {
            tasks = taskRepository.findByCreatedByIdAndPriorityAndCategoryIgnoreCaseAndTaskType(userId, priority, category, TaskType.WORK, pageable);
        } else if (status != null) {
            tasks = taskRepository.findByCreatedByIdAndStatusAndTaskType(userId, status, TaskType.WORK, pageable);
        } else if (priority != null) {
            tasks = taskRepository.findByCreatedByIdAndPriorityAndTaskType(userId, priority, TaskType.WORK, pageable);
        } else if (category != null) {
            tasks = taskRepository.findByCreatedByIdAndCategoryIgnoreCaseAndTaskType(userId, category, TaskType.WORK, pageable);
        } else {
            tasks = taskRepository.findByCreatedByIdAndTaskType(userId, TaskType.WORK, pageable);
        }
    ...
    public Page<TaskResponseDTO> searchTasks(Long userId,
        ...
        Page<Task> titleDescMatches = taskRepository.findByCreatedByIdAndTitleOrDescription(userId, keyword, TaskType.WORK, pageable);
        Page<Task> tagsMatches = taskRepository.findByCreatedByIdAndTagKeyword(userId, keyword, TaskType.WORK, pageable);
        ...
        long finalCount = taskRepository.countByCreatedByIdAndTitleOrDescription(userId, keyword, TaskType.WORK)
                + taskRepository.countByCreatedByIdAndTagKeyword(userId, keyword, TaskType.WORK);
    ...
    public List<TaskResponseDTO> recentTasks(Long userId, int days) {
        ...
        List<Task> tasks = taskRepository.findByCreatedByIdAndLastVisitedAfterAndTaskType(userId, cutoff, TaskType.WORK);
        return tasks.stream()
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }
    ...
    public List<TaskResponseDTO> getDueSoonTasks(Long userId, int days){
        ...
        return taskRepository.findByCreatedByIdAndDueDateBetweenAndTaskType(userId, startOfToday, endOfToday, TaskType.WORK)
                .stream()
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }
    ...
    public Page<TaskResponseDTO> getLibrary(Long userId,
        ...
        Page<Task> tasks = taskRepository.findByCreatedByIdAndTaskType(userId, TaskType.WORK, pageable);
    ...
    private Task getTaskOrThrow(Long userId, Long taskId){
        Task task = taskRepository.findByIdAndCreatedByIdAndTaskType(taskId, userId, TaskType.WORK);
